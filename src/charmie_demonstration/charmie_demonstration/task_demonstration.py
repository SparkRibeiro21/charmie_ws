...
class TaskMain():
    ...
    def main(self):
        ...
        self.MAX_ANGULAR_SPEED = 0.50  # rad/s
                ...
                if new_message and not self.TIMEOUT_FLAG: # if we receive a new message and there is no timeout
                    ...
                    if self.MOTORS_ACTIVE_FLAG: # robot is fully operational 
                        ...
                        if ros2_modules["charmie_low_level"]: # Motors
                            ...
                            # added this compared to navigation_demonstration otherwise 
                            # cmd_vel would be published by the task and by this demonstration
                        ...
                        if ros2_modules["charmie_gamepad"]:
                            ...
                            if len(self.robot.node.received_demo_tsi.list_of_states) != 0:
                                to_send = False
                                if self.robot.get_gamepad_button_pressed(self.robot.BUTTON_DPAD_UP, self.robot.RISING):
                                    self.task_state_selection -= 1
                                    to_send = True
                                if self.robot.get_gamepad_button_pressed(self.robot.BUTTON_DPAD_DOWN, self.robot.RISING):
                                    self.task_state_selection += 1
                                    to_send = True
                                ...
                                if self.robot.get_gamepad_button_pressed(self.robot.BUTTON_DPAD_RIGHT, self.robot.RISING):
                                    self.robot.set_task_state_demo(new_demo_state=temp_state_selection)
                                
                                if self.robot.get_gamepad_button_pressed(self.robot.BUTTON_DPAD_LEFT, self.robot.RISING):
