...
class EGPSRNode(Node):

    def __init__(self):
        super().__init__("EGPSR")
        self.get_logger().info("Initialised CHARMIE EGPSR Node")
...
def main(args=None):
    ...
    node = EGPSRNode()
    th_main = threading.Thread(target=ThreadMainEGPSR, args=(node,), daemon=True)
    th_main.start()
    rclpy.spin(node)
    rclpy.shutdown()

def ThreadMainEGPSR(node: EGPSRNode):
    main = EGPSRMain(node)
    main.main()

class EGPSRMain():

    def __init__(self, node: EGPSRNode):
    ...
    def main(self):
        ...
        # States in EGPSR Task
        self.Waiting_for_task_start = 0
        self.Approach_hallway = 1
        self.Search_hallway = 2
        self.Approach_living_room = 3
        ...
        self.node.get_logger().info("In EGPSR Main...")
        ...
        while True:
            ...
            if self.state == self.Waiting_for_task_start:
                ...
                self.set_speech(filename="gpsr/ready_start_egpsr", wait_for_end_of=True)
                ...
                print("State:", self.state, "- Approach_room1")
                
                self.set_speech(filename="gpsr/moving_hallway", wait_for_end_of=True)
                ...
                self.set_speech(filename="gpsr/arrived_hallway", wait_for_end_of=True)
            ...
            elif self.state == self.Approach_living_room:
                print("State:", self.state, "- Approach_room2")
                
                self.set_speech(filename="gpsr/moving_living_room", wait_for_end_of=True)
                ...
                self.set_speech(filename="gpsr/arrived_living_room", wait_for_end_of=True)

                # next state
                self.state = self.Search_living_room
            ...
            elif self.state == self.Approach_kitchen:
                ...
                # next state
                self.state = self.Search_kitchen
                ...
                self.set_speech(filename="gpsr/finished_egpsr", wait_for_end_of=True)
    ...
    def check_people_waving_in_room(self, local_room, tetas):
        ...
        while still_people_to_analyse:
            ...
            if people_waving_in_same_room > 0:
                self.set_neck_coords(position=[person_waiving.position_absolute.x, person_waiving.position_absolute.y], ang=-10, wait_for_end_of=True)
                self.set_speech(filename="gpsr/detected_egpsr", wait_for_end_of=True)
                self.detected_person_to_face_path(person=person_waiving, send_to_face=True)
                time.sleep(5)
                self.set_speech(filename="gpsr/stand_front_of_me_egpsr", wait_for_end_of=True)
                self.understand_audio_command()
            else:
                still_people_to_analyse = False


    def understand_audio_command(self):
        ### EGPSR EXAMPLE
