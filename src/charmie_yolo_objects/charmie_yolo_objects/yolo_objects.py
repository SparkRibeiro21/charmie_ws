...
class Yolo_obj(Node):
    ...
    def get_color_image_hand_callback(self, img: Image):
        ...
        if self.ACTIVATE_YOLO_DOORS:
            print("Doors Activated - Debug Hand")
            ...
            self.doors_results_hand = self.doors_model.track(current_frame, persist=True, tracker="bytetrack.yaml")
            ...
            num_obj = len(self.doors_results_hand[0])
            ...
            for object_idx in range(num_obj):
                boxes_id = self.doors_results_hand[0].boxes[object_idx]
                # print(self.object_results[0].boxes)

                ALL_CONDITIONS_MET = 1

                object_name = self.door_class_names[int(boxes_id.cls[0])].replace("_", " ").title()
    ...
    def get_color_image_head_callback(self, img: Image):
        ...
        if self.ACTIVATE_YOLO_DOORS:
            print("Doors Activated - Debug Head")
            if not self.waiting_for_pcloud:
                ...
                # The persist=True argument tells the tracker that the current image or frame is the next in a sequence and to expect tracks from the previous image in the current image.
                # results = self.object_model(current_frame, stream = True)
                self.doors_result = self.doors_model.track(current_frame, persist=True, tracker="bytetrack.yaml")

                num_obj = len(self.doors_result[0])
                # self.get_logger().info(f"Objects detected: {num_obj}")

                requested_objects = []
                for object_idx in range(num_obj):

                    boxes_id = self.doors_result[0].boxes[object_idx]
    ...
    def post_receiving_pcloud(self, new_pcloud):
        ...
        if self.ACTIVATE_YOLO_DOORS: 
            ...
            yolov8_obj_filtered.num_objects = num_doors_filtered
            ...
            if self.DEBUG_DRAW:
                ...
                cv2.putText(current_frame_draw, 'np:' + str(num_doors_filtered) + '/' + str(num_obj), (180, self.img_height-10), cv2.FONT_HERSHEY_DUPLEX, 1, (100, 255, 0), 1, cv2.LINE_AA)
            ...
            self.get_logger().info(f"Objects detected: {num_obj}/{num_doors_filtered}")
