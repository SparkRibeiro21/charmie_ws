...
bool charmie_interfaces__msg__multi_objects__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("charmie_interfaces.msg._multi_objects.MultiObjects", full_classname_dest, 50) == 0);
  }
  charmie_interfaces__msg__MultiObjects * ros_message = _ros_message;
  {  // objects
  ...
  {  // distance
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      float * dest = ros_message->distance.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
...
{
  /* NOTE(esteve): Call constructor of MultiObjects */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("charmie_interfaces.msg._multi_objects");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "MultiObjects");
  ...
  {  // distance
    ...
    if (ros_message->distance.size > 0) {
      ...
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      float * src = &(ros_message->distance.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->distance.size * sizeof(float));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
