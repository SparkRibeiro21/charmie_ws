        ...
        try:
            for person_idx, per in enumerate(results[0].keypoints):
                ...
                for keypoint_idx, keypoint in enumerate(keypoints):
                    ...
                    if distance > 0.01 and distance < 6.0:
                ...
                if len(keypoints_distances) > 0:
                    ...
                    #print(results[0].boxes.data[person_idx])
                    x_box_center = int(results[0].boxes.data[person_idx][0]) + int(results[0].boxes.data[person_idx][2] - results[0].boxes.data[person_idx][0])
                    y_box_center = int(results[0].boxes.data[person_idx][1]) + int(results[0].boxes.data[person_idx][3] - results[0].boxes.data[person_idx][1])
                    print(x_box_center)
                    print(y_box_center)
                    person_keypoints.average_distance = (self.cv_image_array[y_box_center, x_box_center]) / 1000  # Note the change in indexing order (y, x)
                    setattr(person_keypoints, "key_p0_x", x_box_center)
                    setattr(person_keypoints, "key_p0_y", y_box_center)
